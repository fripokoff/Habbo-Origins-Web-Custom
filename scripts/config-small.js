...
// var dcr = "https://origins.hiper.esp.br/latest/habbo.dcr?v";
...
function setLoaderbyCountryCode(countryCode) {
	...
	habboEmbed.innerHTML = `<embed id="habboEmbed" src="${loader_config.dcr}" bgcolor="#000000" width="700px" height="900px" swvolume="true"
...
changePageTitleByUrlParameter();

setTimeout(() => {
	setLoaderbyCountryCode(countryCode);
	}, 500);

function changePageTitleByUrlParameter() {
	const queryString = window.location.search;
	const urlParams = new URLSearchParams(queryString);
